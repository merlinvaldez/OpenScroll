...
import { Chip, IconButton, Sheet, Skeleton, Toast } from "./primitives";
...
export default function OpenScrollApp() {
  ...
  const restoreFocus = useRef(null); const messages = catalog[locale]; const isRtl = directionFor(locale) === "rtl";
  useEffect(() => {
    const task = window.setTimeout(() => {
      const saved = parsePreferences(localStorage.getItem(PREFERENCE_KEY));
      setInterest(saved.interest); setSelected(new Set(saved.topics)); setLocale(saved.locale); setTheme(saved.theme); setReady(true);
    }, 0);
  ...
  function chooseInterest(event) { event.preventDefault(); if (interest.trim()) go(1); }
  ...
  function changeLocale(value) { setLocale(value); save({ locale: value }); setToast(catalog[value].language); }
  function cycleTheme() { const next = theme === "dark" ? "light" : "dark"; setTheme(next); save({ theme: next }); }
  if (!ready) return <Skeleton label="Loading preferences"/>;
  return <AppShell locale={locale} theme={theme} messages={messages} feedMode={step === 2} onLocale={changeLocale} onTheme={cycleTheme} onSearch={() => setStep(0)}>
    <main className={`journey ${isRtl ? "journey--rtl" : ""}`}>
      {step === 0 ? <section className="screen opening-screen" aria-labelledby="explore-title"><h1 id="explore-title">{messages.explore}</h1><form className="search-control" onSubmit={chooseInterest}><Search aria-hidden="true"/><label htmlFor="interest" className="sr-only">{messages.interest}</label><input id="interest" value={interest} onChange={(event) => setInterest(event.target.value)} placeholder={messages.placeholder} dir="auto" maxLength={120}/><IconButton className="submit-control" type="submit" disabled={!interest.trim()} label={messages.continue}><ArrowRight className="directional-icon" aria-hidden="true"/></IconButton></form></section> : null}
