...
export default function OpenScrollApp() {
  ...
  function returnToExplore({ focusInput = false } = {}) { restoreFocus.current = focusInput; setStep(0); }
  return <AppShell feedMode={step === 2} messages={messages} onExplore={() => returnToExplore()} onSearch={() => returnToExplore({ focusInput: true })} onSettings={() => setSheet(true)}>
