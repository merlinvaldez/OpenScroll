...
export default function AppShell({ locale, theme, messages, feedMode, onLocale, onTheme, onSearch, children }) {
  return <div className={`app-shell ${feedMode ? "app-shell--feed" : ""}`}>
    {!feedMode ? <aside className="navigation-rail" aria-label="Primary"><span className="brand" aria-label="OpenScroll">O</span><IconButton label={messages.explore} expanded onClick={onSearch}><Compass aria-hidden="true"/></IconButton><IconButton label="Search" expanded onClick={onSearch}><Search aria-hidden="true"/></IconButton><IconButton label="Settings" expanded><Settings aria-hidden="true"/></IconButton><div className="rail-utilities"><label className="locale-select"><Languages aria-hidden="true"/><span className="sr-only">{messages.language}</span><select value={locale} onChange={(event) => onLocale(event.target.value)} aria-label={messages.language}><option value="en">EN</option><option value="es">ES</option><option value="ar">AR</option></select></label><IconButton label={messages.theme} expanded onClick={onTheme}>{theme === "dark" ? <Sun aria-hidden="true"/> : <Moon aria-hidden="true"/>}</IconButton></div></aside> : null}
