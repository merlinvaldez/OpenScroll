...
test("RTL mirrors controls and persists locale", async ({ page }) => { await boot(page); await page.getByLabel("Language").selectOption("ar"); await expect(page.locator("html")).toHaveAttribute("dir","rtl"); await page.reload(); await expect(page.locator("html")).toHaveAttribute("data-hydrated","true"); await expect(page.locator("html")).toHaveAttribute("dir","rtl"); });
test("dark theme and 200 percent zoom remain usable", async ({ page }) => { await boot(page); await page.getByRole("button",{name:"Change theme"}).click(); await expect(page.locator("html")).toHaveAttribute("data-theme","dark"); await page.evaluate(()=>document.body.style.zoom="2"); await expect(page.getByRole("textbox")).toBeVisible(); });
