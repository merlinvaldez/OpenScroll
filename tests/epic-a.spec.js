...
test("RTL mirrors controls and persists locale", async ({ page }) => { await page.addInitScript(() => localStorage.setItem("openscroll:boot-preferences:v1", JSON.stringify({ interest: "", topics: ["Music"], locale: "ar", theme: "system" }))); await boot(page); await expect(page.locator("html")).toHaveAttribute("dir","rtl"); await page.reload(); await expect(page.locator("html")).toHaveAttribute("data-hydrated","true"); await expect(page.locator("html")).toHaveAttribute("dir","rtl"); });
test("dark theme and 200 percent zoom remain usable", async ({ page }) => { await page.addInitScript(() => localStorage.setItem("openscroll:boot-preferences:v1", JSON.stringify({ interest: "", topics: ["Music"], locale: "en", theme: "dark" }))); await boot(page); await expect(page.locator("html")).toHaveAttribute("data-theme","dark"); await page.evaluate(()=>document.body.style.zoom="2"); await expect(page.getByRole("textbox")).toBeVisible(); });
